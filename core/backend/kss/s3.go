...
func (s *S3) newSQSClient() *sqs.Client {
	return sqs.NewFromConfig(s.config, func(o *sqs.Options) {
		if s.sqsEndpointURL != "" {
			o.BaseEndpoint = aws.String(s.sqsEndpointURL)
		}
	})
}
...
func NewS3(kssConfig S3Configuration) (*S3, error) {
	...
	s := S3{
		...
		sqsEndpointURL:       kssConfig.SQSEndpointURL,
	}
	if s.sqsEndpointURL == "" {
		s.sqsEndpointURL = kssConfig.EndpointURL
	}
	if kssConfig.AutoCreateBucket {
		if err := s.ensureBucketAndQueueExist(kssConfig.SkipBucketNotifications); err != nil {
			return nil, err
		}
	}
	if s.sqsQueueName != "" {
...
// then optionally wires S3→SQS bucket notifications and the queue policy that allows S3 to publish.
// This mirrors the CloudFormation NotificationConfiguration + QueuePolicy resources.
// MinIO and ElasticMQ need notification delivery to be configured separately.
func (s *S3) ensureBucketAndQueueExist(skipBucketNotifications bool) error {
	cl := s.newS3Client()
