...
func TestCient(t *testing.T) {
	...
	request := client.Collection("parent/child").WithSelector("child", childID)
	...
	request = client.Collection("parent/child").WithSelector("parent", parentID)
	...
	request = client.Collection("parent/child").WithSelector("child", childID).WithFilter("email", "maybe@yes.no").WithFilter("state", "new")
